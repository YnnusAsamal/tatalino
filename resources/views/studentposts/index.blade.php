    ...
    <div>
      ...
      <section class="essays">
        ...
        <div class="essays-grid">
          ...
          <div class="card mb-3" data-bs-toggle="modal" data-bs-target="#essayModal-{{ $essay->id }}">
              <img src="{{ asset('assets/posts/' . $essay->image) }}" class="card-img-top" alt="Essay Image">
              <div class="card-body">
                  <strong class="card-title">{{ $essay->title }}</strong>
                  <p class="card-text text-muted">{{ $essay->users->name }}</p>
              </div>
          </div>

          <!-- Modal -->
          <div class="modal fade" id="essayModal-{{ $essay->id }}" tabindex="-1" aria-labelledby="essayModalLabel-{{ $essay->id }}" aria-hidden="true">
            <div class="modal-dialog modal-lg modal-dialog-centered">
              <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title" id="essayModalLabel-{{ $essay->id }}">{{ $essay->title }}</h5>
                  <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                  @if($essay->image)
                  <img src="{{ asset('assets/posts/' . $essay->image) }}" class="img-fluid mb-3" alt="Essay Image">
                  @endif
                  <p><strong>Author:</strong> {{ $essay->users->name }}</p>
                  <div>{!! $essay->content !!}</div>
                </div>
                <div class="modal-footer">
                  <small class="text-muted">Published: {{ $essay->created_at->format('F d, Y') }}</small>
